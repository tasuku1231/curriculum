...
-- [SELECT store_name, goods_name, quantity FROM stock_table JOIN store_table ON stock_table.store_code = store_table.store_code RIGHT JOIN goods_table ON stock_table.goods_code = goods_table.goods_code]
...
-- [select store_code, avg(quantity) from stock_table group by store_code; SELECT store_code,COUNT(quantity) FROM stock_table WHERE quantity<17 GROUP BY store_code;]
 
 
-- 6. 商品テーブルに「商品コード='M001'、商品名='マフラー'、単価=4500円、更新日付=本日日付」のデータを追加しなさい。※実行後のSELECT結果も貼付すること。
-- [INSERT INTO goods_table (goods_code, goods_name	, price, update_day) VALUES ('M001', 'マフラー', '4500', '2021-05-18')]
 
 
-- 7. 在庫テーブルの商品コード='S987'、かつ、店舗コード='EA01'に対して、「在庫数=10、更新日付=本日日付」で更新しなさい。※実行後のSELECT結果も貼付すること。
-- [UPDATE stock_table SET quantity = '10', update_day = '2021-05-18' WHERE goods_code = 'S987' and store_code = 'EA01';]
